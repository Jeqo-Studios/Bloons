package net.jeqo.bloons.configuration;

/**
 * A class that contains configurations for the balloons and their entities
 */
public class BalloonConfiguration {
    /**
     * The display name of the armor stand
     */
    public static final String BALLOON_ARMOR_STAND_ID = "4001147";
    /**
     * The display name of the chicken
     */
    public static final String BALLOON_CHICKEN_ID = "4001148";

    /**
     * The identifier for a single balloon type
     */
    public static final String SINGLE_BALLOON_TYPE_IDENTIFIER = "single";
    /**
     * The identifier for a multipart balloon type
     */
    public static final String MULTIPART_BALLOON_TYPE_IDENTIFIER = "multipart";
}
